...
struct ContentView: View {
    @StateObject private var runner = LlamaRunner()
    ...
    var body: some View {
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    Text("Runs a local LLM fully on-device using llama.cpp. Download a small GGUF model, load it, then type a prompt and tap Generate to stream tokens in real time — no internet connection required.")
                        .font(.subheadline)
        ...
        .sheet(isPresented: $isShowingLibrary) {
            ModelLibraryView(downloader: downloader) { url in
                loadModel(url: url)
            }
        }
    ...
    // Load a model that already lives in the app's Documents directory (no security scope needed).
    private func loadModel(url: URL) {
        Task {
            do {
                try await runner.loadModel(url: url)
            } catch {
